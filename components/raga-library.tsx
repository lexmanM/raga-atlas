...
import { ALL_RAGAS, ascent, centsFor, descent, fullScale, JANYA_RAGAS, pitchClass, type Raga, type Swara } from '@/lib/ragas';
...
function Swar({ semitones }: { semitones: number }) {
...
const swarName = (semitones: number) => { const octave = Math.floor(semitones / 12); return `${octave < 0 ? 'lower ' : octave > 0 ? 'upper ' : ''}${SWARS[pitchClass(semitones)].name}`; };
...
export function RagaLibrary({ sruti, onSruti, theme, onTheme, tradition, onTradition }: { sruti: number; onSruti: (value: number) => void; theme: Theme; onTheme: (theme: Theme) => void; tradition: Tradition; onTradition: (tradition: Tradition) => void }) {
  ...
  const selectedId = selectedIds[tradition]; const setSelectedId = (id: string) => setSelectedIds((previous) => ({ ...previous, [tradition]: id }));
  ...
  const switchTradition = (next: Tradition) => { if (next === tradition) return; stop(); onTradition(next); };
  ...
  }, [temperament, voice, kampita, tonic, up, down, octaveOffset, keyboardEnabled]); const play = (direction: 'aro' | 'ava' | 'both' | 'pakad') => { stop(); const sequence = direction === 'pakad' ? (raga.pakad ?? []).flat() : direction === 'aro' ? up : direction === 'ava' ? down : fullScale(raga); setPlaying(direction); setPlayback(playRaga({ sequence, sruti: TONICS[tonic][1], temperament, voice, kampita, tempo, onSwara: (_s, i) => { if (i === -1) setPlaying(''); } })); }; const note = (swara: Swara, direction: string) => { stop(); setPlaying(direction); setPlayback(playRaga({ sequence: [swara], sruti: TONICS[tonic][1], temperament, voice, kampita, tempo, onSwara: (_s, i) => { if (i === -1) setPlaying(''); } })); };
