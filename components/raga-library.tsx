...
export function RagaLibrary({ sruti, onSruti, theme, onTheme, tradition, onTradition }: { sruti: number; onSruti: (value: number) => void; theme: Theme; onTheme: (theme: Theme) => void; tradition: Tradition; onTradition: (tradition: Tradition) => void }) {
  ...
  }, [temperament, voice, kampita, tonic, up, down, octaveOffset, keyboardEnabled]); const play = (direction: 'aro' | 'ava' | 'both' | 'pakad') => { stop(); setInterrupts((count) => count + 1); const sequence = direction === 'pakad' ? (raga.pakad ?? []).flat() : direction === 'aro' ? up : direction === 'ava' ? down : fullScale(raga); setPlaying(direction); setPlayback(playRaga({ sequence, sruti: TONICS[tonic][1], temperament, voice, kampita, tempo, onSwara: (_s, i) => { if (i === -1) setPlaying(''); } })); }; const note = (swara: Swara, direction: string) => { stop(); setInterrupts((count) => count + 1); setPlaying(direction); setPlayback(playRaga({ sequence: [swara], sruti: TONICS[tonic][1], temperament, voice, kampita, tempo, onSwara: (_s, i) => { if (i === -1) setPlaying(''); } })); };
