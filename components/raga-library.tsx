...
function Guitar({ raga, sruti, temperament, tonic }: { raga: Raga; sruti: number; temperament: Temperament; tonic: number }) {
  ...
  return <div className="guitar-container"><div className="guitar-wrapper"><div className="headstock"><div className="headstock-shape" />{Array.from({ length: 6 }, (_, idx) => <div key={idx} className="tuning-peg" />)}</div><div className="fretboard">{guitarTuning.map((open, stringIdx) => <div key={stringIdx} className="string-row"><div className="string-label">{tuningNames[stringIdx]}</div>{Array.from({ length: frets + 1 }, (_, fret) => { const semitone = (open + fret) % 12; const isNote = notes.has(semitone); const baseFreq = freq(semitone, Math.floor((open + fret) / 12)); return <button key={fret} className={`fret ${isNote ? 'note' : ''}`} title={`${LABELS[semitone]} (${baseFreq.toFixed(1)} Hz)`}><span className="fret-label">{isNote ? LABELS[semitone] : (fret === 0 ? '○' : fret % 12 === 0 ? '●' : '')}</span></button>; })}</div>)}</div></div></div>;
}

export function RagaLibrary({ sruti, onSruti }: { sruti: number; onSruti: (value: number) => void }) {
  const [tab, setTab] = useState<'atlas' | 'taxonomy' | 'guitar'>('atlas'); const [filter, setFilter] = useState<'all' | 'melakarta' | 'janya'>('all'); const [selectedId, setSelectedId] = useState('mela-28'); const [tonic, setTonic] = useState(2); const [tempo, setTempo] = useState(2.2); const [playing, setPlaying] = useState(''); const [drone, setDrone] = useState(false); const [temperament, setTemperament] = useState<Temperament>('just'); const [voice, setVoice] = useState<Voice>('veena'); const [kampita, setKampita] = useState(true); const [octaveOffset, setOctaveOffset] = useState(0);
