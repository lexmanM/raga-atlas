...
function Onboarding({ onDismiss }: { onDismiss: () => void }) {
  return <div className="onboarding-overlay" onClick={onDismiss}><div className="onboarding-card" onClick={(e) => e.stopPropagation()}><button className="close-btn" onClick={onDismiss}>✕</button><h2>Welcome to Rāga Atlas</h2><p><strong>Rāgas</strong> are melodic frameworks in Carnatic classical music. Pick one above to explore its structure, hear it played, and see how it maps to guitar.</p><div className="onboarding-tips"><h3>Getting started:</h3><ul><li><strong>Taxonomy</strong> — Learn how ragas are organized</li><li><strong>Atlas</strong> — Deep dive into any raga's details</li><li><strong>Guitar</strong> — See notes on a guitar fretboard</li></ul></div><p style={{fontSize: '14px', marginTop: '16px', color: 'rgba(216,180,94,.7)'}}>Enable Tambura for a reference drone, or use keyboard (a,s,d,f,g,h,j) to play notes.</p><button className="primary" onClick={onDismiss}>Got it</button></div></div>;
}

export function RagaLibrary({ sruti, onSruti }: { sruti: number; onSruti: (value: number) => void }) {
  const [tab, setTab] = useState<'atlas' | 'taxonomy' | 'guitar'>('taxonomy');
